...
class DispatchShell extends Shell {
  ...
  function main() {
    ...
    $projects = $this->Log->find('all', array(
      'conditions' => array(
    ...
    if (isset($projects[0])) {
      ...
      foreach ($projects as $key => $val) {
        ...
        $output = $this->Coderunner->execute($ssh, $val['Instance']['Project']['name'], $val['Instance']['Project']['host'], $destination, $val['Instance']['Project']['path'], $val['Log']['commit'], $branch);
        ...
        if (strpos($output, 'Error:') !== false || strpos($output, 'fatal:') !== false) {
          ...
          $status = 'Completed';
          $output .= $this->Hooks->execute($ssh, $val['Instance']['Server']['server'], $val['Instance']['Project']['name'],  $val['Instance']['Project']['host'], $destination, $val['Instance']['Project']['path'], $val['Log']['commit'], $branch, $val['User']['username']);
