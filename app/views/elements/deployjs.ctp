...
$(document).ready(function() {
  ...
  $('#allservers').click(function() {
    ...
    if (this.checked) {
      $('#instancesTable').find('.commitInfo').css({opacity: 1});
      $('#instancesTable').find('.serverBox .text').css({opacity: 1});
  
    // Dim all the serverboxes if checkbox was unchecked
    } else {
      $('#instancesTable').find('.commitInfo').css({opacity: 0.2});
      $('#instancesTable').find('.serverBox .text').css({opacity: 0.2});
    }
  });
  ...
  $('#instancesTable').find('input:checkbox').click(function(){
    ...
    if (this.checked) {
      $(this).parents('td').children('.commitInfo').css({opacity: 1});
      $(this).parents('td').find('.serverBox .text').css({opacity: 1});
  
    // Dim the chosen serverbox if checkbox was unchecked
    } else {
      $(this).parents('td').children('.commitInfo').css({opacity: 0.2});
      $(this).parents('td').find('.serverBox .text').css({opacity: 0.2});
    }
  });
  
  // Set datepicker on time fields
  $('.timefield').datetimepicker({ dateFormat: 'yy-mm-dd' });
  
  // Gray out the default deploy comment text
  $('.actionBox > .text > input').css({color: '#CCC'});                                                                           
