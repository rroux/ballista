...
class UsersController extends AppController {
  ...
  function login() {
    if ($this->Auth->user()) {
      ...
      $userobj = $this->User->find('first', array('conditions' => array('User.id' => $user['User']['id'])));
      ...
      $acl = $this->User->query("
        ...
        WHERE InstanceGroup.instance_id = Instance.id and InstanceGroup.group_id in (" . implode(',', array_keys($groups)) . ')', 
      $cachequeries = false);
