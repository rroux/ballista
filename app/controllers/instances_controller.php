...
class InstancesController extends AppController {
   ...
   */
	function index() {
    $projects = $this->Instance->Project->find('all', array('order' => array('Project.name' => 'asc'), 'recursive' => 0));
    $servers = $this->Instance->Server->find('all', array('recursive' => 0));
    $groups = $this->Instance->Group->find('all', array('recursive' => 0));
  ...
  function deploy($id = null, $branch = null, $limit = 3) {
    ...
    foreach($instances as $key => $instance){
      ...
      $instances[$key]['lastcommit'] = $this->Instance->Log->find('first', array(
      ...
      $instances[$key]['history'] = $this->Instance->Log->find('all', array(
        'fields' => array('status', 'comment', 'commit', 'updated', 'User.username'),
        'conditions' => array('Log.instance_id' => $instance['Instance']['id'], 'branch' => $branch),
        'order' => array('Log.id DESC'),
        'limit' => 5
      ));
    }
  ...
  function branch_commits($id = null, $branch = null, $serverkey = 0, $limit = 3) {
    $instance = $this->Instance->find('first', array(
      'conditions' => array('Instance.id' => $id),
      'recursive' => 0
    ));
    
    // Find the last commit on the server in this branch
    $lastcommit = $this->Instance->Log->find('first', array(
        'fields' => array('Log.commit'),
        'conditions' => array(
          'instance_id' => $id,
          'branch' => $branch,
    ...
    $instance['history'] = $this->Instance->Log->find('all', array(
      'fields' => array('status', 'comment', 'commit', 'updated', 'User.username'),
      'conditions' => array('Log.instance_id' => $instance['Instance']['id'], 'branch' => $branch),
      'order' => array('Log.id DESC'),
      'limit' => 5
